...
func groupNewHandler(w http.ResponseWriter, r *http.Request) {
	...
	var data Group
	err := error(nil)
	data.CreatorId, err = getRequestSenderIDfromcookie(r)
	if err != nil {
...
func groupJoinHandler(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	defer recovery(w)
	requestorID, err := getRequestSenderIDfromcookie(r)
	if err != nil {
		jsonResponse(w, 401, err.Error())
		return
	}
...
func groupLeaveHandler(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	defer recovery(w)
	requestSenderID, err := getRequestSenderIDfromcookie(r)
	if err != nil {
		jsonResponse(w, 401, err.Error())
		return
	}
...
func groupInvitedHandler(w http.ResponseWriter, r *http.Request) {
	...
	_, err := getRequestSenderIDfromcookie(r)
	if err != nil {
		jsonResponse(w, 401, err.Error())
		return
	}
